# Classification with OTE

This tutorial provide you how to use HPO for classification.
...
```{note}
You should install OTE classification before running OTE.
```
```
ote train \
    training_extensions/external/deep-object-reid/configs/ote_custom_classification/efficientnet_b0/template.yaml \
    --train-ann-files '' \
    --train-data-roots training_extensions/data/classification/train \
    --val-ann-files '' \
    --val-data-roots training_extensions/data/classification/val \
...
When configurations are changed, HPO is excuted without asking.
```
...
Someone who is clever may already have notices,
